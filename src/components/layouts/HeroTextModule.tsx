...
const HeroTextModule: React.FC<Props> = ({
  ...
  return (
    <div
      className={cx(
        `HeroTextModule HeroTextModule--style-${variant} bg-color-${variant} site-max-width site-padding-x mxauto px_75 md:px1_5 xl:px15 pt6 pb2_5 md:pt7_5 md:pb5 ${fontColor}`
      )}
    >
              ...
              return (
                <span key={`HeroTextModule-link-${link.label}`}>
                  <HashLink
                    ...
                    to={link.link}
                  >
                    <span className="hyphens">{link.label}</span>
                  </HashLink>
                  {i !== introBylineLinks.length - 1 && introBylineLinks.length !== 2 && (
                    <span className="vertical-align-middle primary-xxl">
